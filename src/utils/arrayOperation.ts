...
export function isObjectValueEqual<T>(a: T, b: T): boolean {
	...
	let aProps = Object.getOwnPropertyNames(aRecord);
	let bProps = Object.getOwnPropertyNames(bRecord);
	if (aProps.length != bProps.length) return false;
	for (let i = 0; i < aProps.length; i++) {
		let propName = aProps[i];
		let propA = aRecord[propName];
		let propB = bRecord[propName];
		if (!Object.prototype.hasOwnProperty.call(bRecord, propName)) return false;
	...
	if (!Object.keys(arr).length) {
		...
		if (attr) {
			...
			return arr.reduce((cur: EmptyArrayType[], item: EmptyArrayType) => {
				...
				const key = String(record[attr] ?? '');
				obj[key] ? '' : (obj[key] = true && Boolean(record[attr]) && cur.push(item));
				return cur;
			}, []);
		} else {
			return [...new Set(arr)];
