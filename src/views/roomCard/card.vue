	...
	<div class="page">
		<el-card shadow="hover" :body-style="{ paddingBottom: '0' }">
			<el-form :model="queryParams" ref="queryForm" :inline="true">
				<el-form-item label="搜索圈主 ">
					<el-input v-model="queryParams.arg" clearable="" style="width: 200px" placeholder="请输入名称或ID" />
				</el-form-item>
		...
		<el-card class="full-table" shadow="hover">
			<el-table :data="tableData" v-loading="loading" tooltip-effect="light" row-key="id" border="">
				...
				<el-table-column label="ID" :formatter="(e: any) => (e.ppk || '').replace('0000100000', '')" />
				...
				<el-table-column label="俱乐部ID" :formatter="(e: any) => (e.clubKey || '').replace('0000100000', '')" min-width="100" />
				<el-table-column prop="phoneNumber" label="电话号码" min-width="100" />
				<el-table-column prop="clubName" label="俱乐部名称" min-width="120" />
				<el-table-column prop="costRoomCardNum" label="房卡统计" min-width="200">
					<template #default="{ row }">
				...
				<el-table-column prop="clubOnlineNum" label="俱乐部在线人数" min-width="150">
					<template #default="{ row }">
						<p>在线人数：{{ row.clubOnlineNum }}</p>
						<p>总人数：{{ row.clubTotalNum }}</p>
					</template>
				</el-table-column>
				<el-table-column prop="clubFreeRoomCardTimes" label="免费时间段" min-width="240">
					<template #default="{ row }">
						<p>开始时间：{{ setTime(row.clubFreeRoomCardTimes?.[0] || 0) }}</p>
						<p>结束时间：{{ setTime(row.clubFreeRoomCardTimes?.[1] || 0) }}</p>
					</template>
				</el-table-column>
				...
				<el-table-column label="操作" min-width="140" align="center" fixed="right">
					<template #default="{ row }">
						<el-dropdown>
							...
							<template #dropdown>
								<el-dropdown-menu>
									<el-dropdown-item @click="memberDiaRef.show(row)">成员列表</el-dropdown-item>
...
import { GetBit, setTime } from '/@/config/tool';
...
const handleQuery = async () => {
	...
	try {
		const res = await $http('/admin/getRoomCardCostRecords', {
			...
			arg: queryParams.value.arg || null, // 指定查询参数(名称或id字符串 此参数可传可不传)
			clubModel: queryParams.value.type === 2 ? queryParams.value.clubModel || null : null,
		});
		tableData.value = res.array ?? [];
		tableParams.value.total = res.total ?? 0;
	} catch (error) {
		reportAdminError(error, 'roomCard.cardQuery', '房卡记录加载失败，请重试');
	}
	loading.value = false;
};
